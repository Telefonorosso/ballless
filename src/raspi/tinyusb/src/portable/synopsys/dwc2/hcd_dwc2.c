...
#if defined(EMU68_TINYUSB_DWC2)
extern void emu68_usbmouse_diag_port_reset(uint8_t end_phase);
extern void emu68_usbmouse_diag_setup(uint8_t dev_addr, uint8_t const setup[8]);
extern void emu68_usbmouse_diag_hcint(uint8_t ch, uint32_t hcint, uint32_t hcchar, uint32_t hctsiz,
                                      uint32_t hcsplt, uint32_t hcintmsk,
                                      uint8_t root_speed, uint8_t dev_speed);
extern void emu68_usbmouse_diag_nptxfe(uint32_t gintsts, uint32_t gintmsk, uint32_t hnptxsts, uint32_t hcchar, uint32_t hctsiz, uint32_t hcintmsk);

static inline volatile uint32_t *emu68_dwc2_fifo_ptr(dwc2_regs_t *dwc2, uint8_t fifo_num)
{
  return (volatile uint32_t *)(uintptr_t)((uintptr_t)dwc2 + 0x1000u + ((uintptr_t)fifo_num * 0x1000u));
}
static void emu68_dwc2_fifo_read(dwc2_regs_t *dwc2, uint8_t fifo_num, uint8_t *dst, uint16_t len)
{
  volatile uint32_t *fifo = emu68_dwc2_fifo_ptr(dwc2, fifo_num);
  while (len) {
    uint32_t w = __builtin_bswap32(*fifo);
    uint16_t n = len > 4u ? 4u : len;
    for (uint16_t i = 0; i < n; ++i) dst[i] = (uint8_t)(w >> (8u * i));
    dst += n; len -= n;
  }
}
static void emu68_dwc2_fifo_write(dwc2_regs_t *dwc2, uint8_t fifo_num, const uint8_t *src, uint16_t len)
{
  volatile uint32_t *fifo = emu68_dwc2_fifo_ptr(dwc2, fifo_num);
  while (len) {
    uint32_t w = 0; uint16_t n = len > 4u ? 4u : len;
    for (uint16_t i = 0; i < n; ++i) w |= ((uint32_t)src[i]) << (8u * i);
    *fifo = __builtin_bswap32(w);
    src += n; len -= n;
  }
}
#endif
...
/*
 * Emu68 is AArch64 big-endian. DWC2 register accesses are byte-swapped by
 * scalar_storage_order, but C bitfield layout is still target-endian and must
 * NOT be used to encode/decode host-channel register words.
 *
 * Keep cached HCCHAR/HCSPLT values in their architectural numeric layout and
 * manipulate fields only with masks/shifts.
 */
static inline uint32_t emu68_field_get(uint32_t v, uint32_t mask, unsigned pos)
{
  return (v & mask) >> pos;
}

static inline uint32_t emu68_field_put(uint32_t v, uint32_t mask, unsigned pos, uint32_t x)
{
  return (v & ~mask) | ((x << pos) & mask);
}

static inline uint16_t emu68_hcchar_mps(uint32_t v)
{
  return (uint16_t)emu68_field_get(v, HCCHAR_MPSIZ_Msk, HCCHAR_MPSIZ_Pos);
}
static inline uint8_t emu68_hcchar_epnum(uint32_t v)
{
  return (uint8_t)emu68_field_get(v, HCCHAR_EPNUM_Msk, HCCHAR_EPNUM_Pos);
}
static inline uint8_t emu68_hcchar_dir(uint32_t v)
{
  return (uint8_t)emu68_field_get(v, HCCHAR_EPDIR_Msk, HCCHAR_EPDIR_Pos);
}
static inline uint8_t emu68_hcchar_type(uint32_t v)
{
  return (uint8_t)emu68_field_get(v, HCCHAR_EPTYP_Msk, HCCHAR_EPTYP_Pos);
}
static inline uint8_t emu68_hcchar_addr(uint32_t v)
{
  return (uint8_t)emu68_field_get(v, HCCHAR_DAD_Msk, HCCHAR_DAD_Pos);
}
static inline bool emu68_hcchar_enabled(uint32_t v)
{
  return (v & HCCHAR_CHENA) != 0;
}
static inline uint32_t emu68_hcchar_set_dir(uint32_t v, uint8_t dir)
{
  return emu68_field_put(v, HCCHAR_EPDIR_Msk, HCCHAR_EPDIR_Pos, dir);
}
static inline uint32_t emu68_hcchar_set_odd(uint32_t v, uint8_t odd)
{
  return emu68_field_put(v, HCCHAR_ODDFRM_Msk, HCCHAR_ODDFRM_Pos, odd);
}

static inline uint32_t emu68_hctsiz_make(uint32_t size, uint32_t packets,
                                         uint32_t pid, bool do_ping)
{
  uint32_t v = 0;
  v = emu68_field_put(v, HCTSIZ_XFRSIZ_Msk, HCTSIZ_XFRSIZ_Pos, size);
  v = emu68_field_put(v, HCTSIZ_PKTCNT_Msk, HCTSIZ_PKTCNT_Pos, packets);
  v = emu68_field_put(v, HCTSIZ_PID_Msk, HCTSIZ_PID_Pos, pid);
  if (do_ping) v |= HCTSIZ_DOPING;
  return v;
}
static inline uint32_t emu68_hctsiz_pid(uint32_t v)
{
  return emu68_field_get(v, HCTSIZ_PID_Msk, HCTSIZ_PID_Pos);
}
static inline uint16_t emu68_hctsiz_packets(uint32_t v)
{
  return (uint16_t)emu68_field_get(v, HCTSIZ_PKTCNT_Msk, HCTSIZ_PKTCNT_Pos);
}
static inline uint32_t emu68_hctsiz_size(uint32_t v)
{
  return emu68_field_get(v, HCTSIZ_XFRSIZ_Msk, HCTSIZ_XFRSIZ_Pos);
}

static inline bool emu68_hcsplt_enabled(uint32_t v)
{
  return (v & HCSPLT_SPLITEN_Msk) != 0;
}
static inline bool emu68_hcsplt_complete(uint32_t v)
{
  return (v & HCSPLT_COMPLSPLT_Msk) != 0;
}
static inline uint32_t emu68_hcsplt_set_complete(uint32_t v, bool complete)
{
  return complete ? (v | HCSPLT_COMPLSPLT_Msk) : (v & ~HCSPLT_COMPLSPLT_Msk);
}
static inline uint32_t emu68_hcsplt_make(uint8_t hub_port, uint8_t hub_addr,
                                         bool split_enable)
{
  uint32_t v = 0;
  v = emu68_field_put(v, HCSPLT_PRTADDR_Msk, HCSPLT_PRTADDR_Pos, hub_port);
  v = emu68_field_put(v, HCSPLT_HUBADDR_Msk, HCSPLT_HUBADDR_Pos, hub_addr);
  if (split_enable) v |= HCSPLT_SPLITEN_Msk;
  return v;
}

/* HPTXSTS and HNPTXSTS share FIFO-available[15:0] and queue-available[23:16]. */
static inline uint16_t emu68_txsts_fifo_available(uint32_t v)
{
  return (uint16_t)(v & 0xffffu);
}
static inline uint8_t emu68_txsts_queue_available(uint32_t v)
{
...
TU_ATTR_ALWAYS_INLINE static inline tusb_speed_t hprt_speed_get(dwc2_regs_t* dwc2) {
  ...
  const uint32_t hprt_speed = (hprt & HPRT_SPEED_Msk) >> HPRT_SPEED_Pos;
  switch(hprt_speed) {
...
TU_ATTR_ALWAYS_INLINE static inline bool channel_is_periodic(uint32_t hcchar) {
  const uint8_t ep_type = emu68_hcchar_type(hcchar);
...
TU_ATTR_ALWAYS_INLINE static inline uint8_t req_queue_avail(const dwc2_regs_t* dwc2, bool is_period) {
  const uint32_t txsts = is_period ? dwc2->hptxsts : dwc2->hnptxsts;
  return emu68_txsts_queue_available(txsts);
}
...
TU_ATTR_ALWAYS_INLINE static inline uint8_t channel_find_enabled(dwc2_regs_t* dwc2, uint8_t dev_addr, uint8_t ep_num, uint8_t ep_dir) {
  ...
  for (uint8_t ch_id = 0; ch_id < max_channel; ch_id++) {
    if (_hcd_data.xfer[ch_id].allocated) {
      const uint32_t hcchar = dwc2->channel[ch_id].hcchar;
      if (emu68_hcchar_addr(hcchar) == dev_addr && emu68_hcchar_epnum(hcchar) == ep_num &&
          (ep_num == 0 || emu68_hcchar_dir(hcchar) == ep_dir)) {
        return ch_id;
      }
    }
  }
...
TU_ATTR_ALWAYS_INLINE static inline uint8_t edpt_alloc(void) {
  for (uint32_t i = 0; i < CFG_TUH_DWC2_ENDPOINT_MAX; i++) {
    ...
    if (!emu68_hcchar_enabled(edpt->hcchar)) {
      tu_memclr(edpt, sizeof(hcd_endpoint_t));
      edpt->hcchar |= HCCHAR_CHENA;
      return i;
    }
  }
  return TUSB_INDEX_INVALID_8;
...
TU_ATTR_ALWAYS_INLINE static inline uint8_t edpt_find_opened(uint8_t dev_addr, uint8_t ep_num, uint8_t ep_dir) {
  for (uint8_t i = 0; i < (uint8_t)CFG_TUH_DWC2_ENDPOINT_MAX; i++) {
    ...
    const uint32_t hcchar = edpt->hcchar;
    if (emu68_hcchar_enabled(hcchar) && emu68_hcchar_addr(hcchar) == dev_addr &&
        emu68_hcchar_epnum(hcchar) == ep_num &&
        (ep_num == 0 || emu68_hcchar_dir(hcchar) == ep_dir)) {
      return i;
    }
  }
...
#if defined(EMU68_TINYUSB_DWC2)
  emu68_usbmouse_diag_port_reset(0);
#endif
  dwc2_regs_t* dwc2 = DWC2_REG(rhport);
...
#if defined(EMU68_TINYUSB_DWC2)
  emu68_usbmouse_diag_port_reset(1);
#endif
  dwc2_regs_t* dwc2 = DWC2_REG(rhport);
...
void hcd_device_close(uint8_t rhport, uint8_t dev_addr) {
  ...
  for (uint8_t ep_id = 0; ep_id < CFG_TUH_DWC2_ENDPOINT_MAX; ep_id++) {
    ...
    if (emu68_hcchar_enabled(edpt->hcchar) && emu68_hcchar_addr(edpt->hcchar) == dev_addr) {
...
bool hcd_edpt_open(uint8_t rhport, uint8_t dev_addr, const tusb_desc_endpoint_t* desc_ep) {
  ...
  uint32_t hcchar = 0;
  hcchar = emu68_field_put(hcchar, HCCHAR_MPSIZ_Msk, HCCHAR_MPSIZ_Pos, tu_edpt_packet_size(desc_ep));
  hcchar = emu68_field_put(hcchar, HCCHAR_EPNUM_Msk, HCCHAR_EPNUM_Pos, tu_edpt_number(desc_ep->bEndpointAddress));
  hcchar = emu68_field_put(hcchar, HCCHAR_EPDIR_Msk, HCCHAR_EPDIR_Pos, tu_edpt_dir(desc_ep->bEndpointAddress));
  if (bus_info.speed == TUSB_SPEED_LOW) hcchar |= HCCHAR_LSDEV_Msk;
  hcchar = emu68_field_put(hcchar, HCCHAR_EPTYP_Msk, HCCHAR_EPTYP_Pos, desc_ep->bmAttributes.xfer);
  hcchar = emu68_field_put(hcchar, HCCHAR_DAD_Msk, HCCHAR_DAD_Pos, dev_addr);
  hcchar |= HCCHAR_CHENA; /* cached allocation marker; cleared before programming hardware */
  edpt->hcchar = hcchar;

  edpt->hcsplt = emu68_hcsplt_make(bus_info.hub_port, bus_info.hub_addr,
...
static void channel_xfer_out_wrapup(dwc2_regs_t* dwc2, uint8_t ch_id) {
  ...
  const uint32_t hctsiz = channel->hctsiz;
  edpt->next_pid = (uint8_t)emu68_hctsiz_pid(hctsiz); // save PID
   ...
   */
  const uint16_t remain_packets = emu68_hctsiz_packets(hctsiz);
  const uint16_t ep_size = emu68_hcchar_mps(channel->hcchar);
  const uint16_t total_packets = cal_packet_count(edpt->buflen, ep_size);
  const uint16_t actual_bytes = (total_packets - remain_packets) * ep_size;
...
static bool channel_xfer_start(dwc2_regs_t* dwc2, uint8_t ch_id) {
  ...
  if (is_period) {
    edpt->hcchar = emu68_hcchar_set_odd(edpt->hcchar, (uint8_t)(1 - (dwc2->hfnum & 1)));
  }
  channel->hcchar = (edpt->hcchar & ~HCCHAR_CHENA);

  // hctsiz: zero length packet still counts as 1
  const uint16_t ep_size = emu68_hcchar_mps(edpt->hcchar);
  const uint8_t ep_dir = emu68_hcchar_dir(edpt->hcchar);
  const uint16_t packet_count = cal_packet_count(edpt->buflen, ep_size);
  const bool do_ping = edpt->next_do_ping && edpt->speed == TUSB_SPEED_HIGH &&
                       edpt->next_pid != HCTSIZ_PID_SETUP && ep_dir == TUSB_DIR_OUT;
  const uint32_t hctsiz = emu68_hctsiz_make(edpt->buflen, packet_count, edpt->next_pid, do_ping);
  ...
  if (emu68_hcchar_epnum(edpt->hcchar) == 0) {
  ...
  if (dma_host_enabled(dwc2)) {
    ...
    if (ep_dir == TUSB_DIR_IN) {
      ...
      hcintmsk |= HCINT_NYET;
      if (emu68_hcsplt_enabled(edpt->hcsplt) || do_ping) {
        hcintmsk |= HCINT_ACK;
      }
    }
...
bool hcd_edpt_xfer(uint8_t rhport, uint8_t dev_addr, uint8_t ep_addr, uint8_t * buffer, uint16_t buflen) {
  ...
  if (ep_num == 0) {
    ...
    edpt->hcchar = emu68_hcchar_set_dir(edpt->hcchar, ep_dir);
  }
...
#if defined(EMU68_TINYUSB_DWC2)
  emu68_usbmouse_diag_setup(dev_addr, setup_packet);
#endif
...
static void channel_xfer_in_retry(dwc2_regs_t* dwc2, uint8_t ch_id, uint32_t hcint) {
  ...
  if (channel_is_periodic(hcchar)){
    uint32_t hcsplt = channel->hcsplt;
    // retry immediately for periodic split NYET if we haven't reach max retry
    if (emu68_hcsplt_enabled(hcsplt) && emu68_hcsplt_complete(hcsplt) &&
        (hcint & HCINT_NYET || xfer->halted_nyet)) {
      xfer->period_split_nyet_count++;
      xfer->halted_nyet = 0;
      if (xfer->period_split_nyet_count < HCD_XFER_PERIOD_SPLIT_NYET_MAX) {
        hcchar = emu68_hcchar_set_odd(hcchar, (uint8_t)(1 - (dwc2->hfnum & 1)));
    ...
    if (edpt->uframe_interval == ucount) {
      ...
      edpt->next_pid = (uint8_t)emu68_hctsiz_pid(channel->hctsiz); // save PID
...
static void handle_rxflvl_irq(uint8_t rhport) {
  ...
  //
  // Emu68 is AArch64 big-endian. The DWC2 MMIO word itself is converted
  // correctly by scalar_storage_order, but C bitfield layout must not be
  // used to decode the architectural GRXSTSP fields.
  //
  // GRXSTSP host layout:
  //   [3:0]   channel number
  //   [14:4]  byte count
  ...
  //   [20:17] packet status
  //   [24:21] frame number
  const uint32_t grxstsp = dwc2->grxstsp;
  const uint8_t ch_id = (uint8_t)(grxstsp & 0x0fu);
  const uint16_t byte_count = (uint16_t)((grxstsp >> 4) & 0x07ffu);
  const uint8_t packet_status = (uint8_t)((grxstsp >> 17) & 0x0fu);

  switch (packet_status) {
    case GRXSTS_PKTSTS_RX_DATA: {
      // IN packet received, pop its payload from FIFO0.
      hcd_xfer_t* xfer = &_hcd_data.xfer[ch_id];
      ...
      if (byte_count > 0) {
#if defined(EMU68_TINYUSB_DWC2)
        emu68_dwc2_fifo_read(dwc2, 0, edpt->buffer + xfer->xferred_bytes, byte_count);
#else
...
static bool handle_txfifo_empty(dwc2_regs_t* dwc2, bool is_periodic) {
  ...
  for (uint8_t ch_id = 0; ch_id < max_channel; ch_id++) {
    ...
    if (0 == (channel->hcintmsk & HCINT_HALTED) && (emu68_hcchar_dir(hcchar) == TUSB_DIR_OUT)) {
      hcd_xfer_t *xfer = &_hcd_data.xfer[ch_id];
      TU_ASSERT(xfer->ep_id < CFG_TUH_DWC2_ENDPOINT_MAX);
      hcd_endpoint_t* edpt = &_hcd_data.edpt[xfer->ep_id];
      const uint16_t remain_packets = emu68_hctsiz_packets(channel->hctsiz);
      for (uint16_t i = 0; i < remain_packets; i++) {
        const uint16_t remain_bytes = edpt->buflen - xfer->fifo_bytes;
        const uint16_t xact_bytes = tu_min16(remain_bytes, emu68_hcchar_mps(hcchar));
        ...
        const uint32_t txsts = is_periodic ? dwc2->hptxsts : dwc2->hnptxsts;
        if ((xact_bytes > ((uint32_t)emu68_txsts_fifo_available(txsts) << 2)) ||
            (emu68_txsts_queue_available(txsts) == 0)) {
          return true;
        }

#if defined(EMU68_TINYUSB_DWC2)
        emu68_dwc2_fifo_write(dwc2, ch_id, edpt->buffer + xfer->fifo_bytes, xact_bytes);
#else
...
static bool handle_channel_in_slave(dwc2_regs_t* dwc2, uint8_t ch_id, uint32_t hcint) {
  ...
  if (hcint & HCINT_XFER_COMPLETE) {
    if (emu68_hcchar_epnum(edpt->hcchar) != 0) {
      edpt->next_pid = (uint8_t)emu68_hctsiz_pid(hctsiz); // save pid (already toggled)
    }

    const uint16_t remain_packets = emu68_hctsiz_packets(hctsiz);
    if (emu68_hcsplt_enabled(hcsplt) && remain_packets &&
        xfer->fifo_bytes == emu68_hcchar_mps(edpt->hcchar)) {
      // Split can only complete 1 transaction (up to 1 packet) at a time, schedule more
      hcsplt = emu68_hcsplt_set_complete(hcsplt, false);
      channel->hcsplt = hcsplt;
    } else {
    ...
    hcsplt = emu68_hcsplt_set_complete(hcsplt, true);
    channel->hcsplt = hcsplt;
    xfer->halted_nyet = 1;
    channel_disable(dwc2, channel);
  } else if (hcint & HCINT_NAK) {
    // NAK received, disable channel to flush all posted request and try again
    if (emu68_hcsplt_enabled(hcsplt)) {
      hcsplt = emu68_hcsplt_set_complete(hcsplt, false); // restart with start-split
      channel->hcsplt = hcsplt;
    }
    ...
    if (emu68_hcsplt_enabled(hcsplt)) {
      if (!emu68_hcsplt_complete(hcsplt)) {
    ...
    if (tu_bit_test(dwc2->haint, ch_id)) {
...
#if defined(EMU68_TINYUSB_DWC2)
      emu68_usbmouse_diag_hcint(ch_id, hcint, channel->hcchar, channel->hctsiz,
                                channel->hcsplt, channel->hcintmsk,
                                (uint8_t) hprt_speed_get(dwc2),
                                (uint8_t) _hcd_data.edpt[xfer->ep_id].speed);
#endif

      bool is_done = false;
      if (is_dma) {
        #if CFG_TUH_DWC2_DMA_ENABLE
        if (emu68_hcchar_dir(hcchar) == TUSB_DIR_OUT) {
      ...
      if (is_done) {
        if (xfer->closing == 1) {
          ...
          const uint8_t ep_addr = tu_edpt_addr(emu68_hcchar_epnum(hcchar), emu68_hcchar_dir(hcchar));
          hcd_event_xfer_complete(emu68_hcchar_addr(hcchar), ep_addr, xfer->xferred_bytes,
                                  (xfer_result_t)xfer->result, in_isr);
        }
        channel_dealloc(dwc2, ch_id);
      }
    }
...
static bool handle_sof_irq(uint8_t rhport, bool in_isr) {
  ...
  for(uint8_t ep_id = 0; ep_id < CFG_TUH_DWC2_ENDPOINT_MAX; ep_id++) {
    hcd_endpoint_t *edpt = &_hcd_data.edpt[ep_id];
    if (edpt->closing == 0) {
      if (emu68_hcchar_enabled(edpt->hcchar) && channel_is_periodic(edpt->hcchar) && edpt->uframe_countdown > 0) {
...
static void handle_hprt_irq(uint8_t rhport, bool in_isr) {
  ...
  /*
   * Emu68 POC4: do not use dwc2_hprt_t C bitfields here.
   * Emu68 runs AArch64 BE while DWC2 MMIO is LE; the MMIO word itself is
   * converted correctly by dwc2_regs_t, but C bitfield allocation order is
   * target/compiler dependent. Use the canonical HPRT masks instead.
   */
  const uint32_t hprt_raw = dwc2->hprt;
  uint32_t hprt = hprt_raw & ~HPRT_W1_MASK;

  if ((hprt_raw & HPRT_CONN_DETECT) != 0u) {
    // Port Connect Detect (W1C)
    hprt |= HPRT_CONN_DETECT;

    if ((hprt_raw & HPRT_CONN_STATUS) != 0u) {
      hcd_event_device_attach(rhport, in_isr);
    }
  }

  if ((hprt_raw & HPRT_ENABLE_CHANGE) != 0u) {
    // Port enable change (W1C)
    hprt |= HPRT_ENABLE_CHANGE;

    if ((hprt_raw & HPRT_ENABLE) != 0u) {
      const tusb_speed_t speed = hprt_speed_get(dwc2);
  ...
  dwc2->hprt = hprt; // clear handled W1C interrupt bits
}
...
#if defined(EMU68_TINYUSB_DWC2)
    emu68_usbmouse_diag_nptxfe(dwc2->gintsts, dwc2->gintmsk, dwc2->hnptxsts,
                               dwc2->channel[0].hcchar, dwc2->channel[0].hctsiz,
                               dwc2->channel[0].hcintmsk);
#endif
