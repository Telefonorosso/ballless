...
/*
 * Diagnostic splash/tracing is retained below for troubleshooting but is
 * disabled in the normal milestone build.  Set to 1 to restore it.
 */
#ifndef EMU68_USBMOUSE_DIAGNOSTICS
#define EMU68_USBMOUSE_DIAGNOSTICS 0
#endif

extern void emu68_usbmouse_init(void);
extern void emu68_usbmouse_poll(void);
extern void emu68_usbmouse_clear_motion(void);
extern volatile uint8_t  emu68_usbmouse_host_init_ok;
extern volatile uint8_t  emu68_usbmouse_host_mode;
extern volatile uint8_t  emu68_usbmouse_port_connected;
extern volatile uint8_t  emu68_usbmouse_enum_mounted;
extern volatile uint8_t  emu68_usbmouse_enum_addr;
extern volatile uint8_t  emu68_usbmouse_mounted;
extern volatile uint32_t emu68_usbmouse_report_count;
extern volatile int8_t   emu68_usbmouse_last_dx;
extern volatile int8_t   emu68_usbmouse_last_dy;
extern volatile uint32_t emu68_usbmouse_gsnpsid;
extern volatile uint8_t  emu68_usbmouse_reset_started;
extern volatile uint8_t  emu68_usbmouse_reset_ended;
extern volatile uint32_t emu68_usbmouse_setup_count;
extern volatile uint8_t  emu68_usbmouse_setup_dev;
extern volatile uint8_t  emu68_usbmouse_setup_bmreq;
extern volatile uint8_t  emu68_usbmouse_setup_breq;
extern volatile uint16_t emu68_usbmouse_setup_wvalue;
extern volatile uint16_t emu68_usbmouse_setup_windex;
extern volatile uint16_t emu68_usbmouse_setup_wlength;
extern volatile uint32_t emu68_usbmouse_hcint_count;
extern volatile uint8_t  emu68_usbmouse_last_ch;
extern volatile uint32_t emu68_usbmouse_last_hcint;
extern volatile uint32_t emu68_usbmouse_last_hcchar;
extern volatile uint32_t emu68_usbmouse_last_hctsiz;
extern volatile uint32_t emu68_usbmouse_hprt;
extern volatile uint32_t emu68_usbmouse_gintsts;
extern volatile uint32_t emu68_usbmouse_gintmsk;
extern volatile uint32_t emu68_usbmouse_hnptxsts;
extern volatile uint32_t emu68_usbmouse_nptxfe_count;
extern volatile uint32_t emu68_usbmouse_nptxfe_gintsts;
extern volatile uint32_t emu68_usbmouse_nptxfe_gintmsk;
extern volatile uint32_t emu68_usbmouse_hc0_hcchar;
extern volatile uint32_t emu68_usbmouse_hc0_hctsiz;
extern volatile uint32_t emu68_usbmouse_hc0_hcintmsk;
extern volatile uint32_t emu68_usbmouse_hc_trace_count;
extern volatile uint32_t emu68_usbmouse_hc_trace_int[8];
extern volatile uint32_t emu68_usbmouse_hc_trace_char[8];
extern volatile uint32_t emu68_usbmouse_hc_trace_size[8];
extern volatile uint32_t emu68_usbmouse_hc_trace_split[8];
extern volatile uint32_t emu68_usbmouse_hc_trace_mask[8];
extern volatile uint8_t  emu68_usbmouse_hc_trace_root_speed[8];
extern volatile uint8_t  emu68_usbmouse_hc_trace_dev_speed[8];
extern volatile uint32_t emu68_usbmouse_hc0_hcsplt;

#if EMU68_USBMOUSE_DIAGNOSTICS
static void usbmouse_diag_poll_ms(uint32_t ms)
{
    ...
    } while (((now - start) * 1000ULL) < ((uint64_t)ms * freq));
}

static void usbmouse_splash_diagnostic(void)
{
    /* Give enumeration a full ten-second event-driven window. */
    text_x = 0;
    text_y = 0;
    kprintf_pc(__putc, NULL, "USB MOUSE HOST DIAGNOSTIC\n");
    kprintf_pc(__putc, NULL, "Move the USB mouse now...\n\n");

    usbmouse_diag_poll_ms(10000);

    text_x = 0;
    text_y = 3;
    kprintf_pc(__putc, NULL, "DWC2 ID       : %08x\n", emu68_usbmouse_gsnpsid);
    kprintf_pc(__putc, NULL, "HOST INIT     : %s\n", emu68_usbmouse_host_init_ok ? "OK" : "FAIL");
    kprintf_pc(__putc, NULL, "HOST MODE     : %s\n", emu68_usbmouse_host_mode ? "OK" : "FAIL");
    kprintf_pc(__putc, NULL, "PORT CONNECT  : %s\n", emu68_usbmouse_port_connected ? "OK" : "NO");
    kprintf_pc(__putc, NULL, "PORT RESET    : %s/%s\n",
               emu68_usbmouse_reset_started ? "START" : "NO",
               emu68_usbmouse_reset_ended ? "END" : "NO");
    kprintf_pc(__putc, NULL, "SETUP COUNT   : %u\n", emu68_usbmouse_setup_count);
    kprintf_pc(__putc, NULL, "LAST SETUP    : d=%u bm=%02x req=%02x\n",
               (uint32_t)emu68_usbmouse_setup_dev, (uint32_t)emu68_usbmouse_setup_bmreq,
               (uint32_t)emu68_usbmouse_setup_breq);
    kprintf_pc(__putc, NULL, "wValue/Idx/Len: %04x %04x %04x\n",
               (uint32_t)emu68_usbmouse_setup_wvalue, (uint32_t)emu68_usbmouse_setup_windex,
               (uint32_t)emu68_usbmouse_setup_wlength);
    kprintf_pc(__putc, NULL, "HCINT EVENTS  : %u last ch=%u last=%08x\n",
               emu68_usbmouse_hcint_count, (uint32_t)emu68_usbmouse_last_ch,
               emu68_usbmouse_last_hcint);
    kprintf_pc(__putc, NULL, "HPRT/GINTSTS  : %08x %08x\n",
               emu68_usbmouse_hprt, emu68_usbmouse_gintsts);
    kprintf_pc(__putc, NULL, "GINTMSK/HNPTX : %08x %08x\n",
               emu68_usbmouse_gintmsk, emu68_usbmouse_hnptxsts);
    kprintf_pc(__putc, NULL, "NPTXFE SERVICE: %u\n", emu68_usbmouse_nptxfe_count);
    kprintf_pc(__putc, NULL, "HC0 C/S/Z/M   : %08x %08x %08x %08x\n",
               emu68_usbmouse_hc0_hcchar, emu68_usbmouse_hc0_hcsplt,
               emu68_usbmouse_hc0_hctsiz, emu68_usbmouse_hc0_hcintmsk);
    kprintf_pc(__putc, NULL, "HC0 EVENT TRACE (%u):\n", emu68_usbmouse_hc_trace_count);
    {
        ...
        for (uint32_t i = 0; i < n; ++i) {
            kprintf_pc(__putc, NULL,
                       "%u I=%08x C=%08x Z=%08x\n",
                       i + 1U, emu68_usbmouse_hc_trace_int[i],
                       emu68_usbmouse_hc_trace_char[i],
                       emu68_usbmouse_hc_trace_size[i]);
            kprintf_pc(__putc, NULL,
                       "  S=%08x M=%08x R/D=%u/%u\n",
                       emu68_usbmouse_hc_trace_split[i],
                       emu68_usbmouse_hc_trace_mask[i],
                       (uint32_t)emu68_usbmouse_hc_trace_root_speed[i],
                       (uint32_t)emu68_usbmouse_hc_trace_dev_speed[i]);
        }
    }
    kprintf_pc(__putc, NULL, "USB ENUM      : %s", emu68_usbmouse_enum_mounted ? "OK" : "NO");
    if (emu68_usbmouse_enum_mounted)
        kprintf_pc(__putc, NULL, " addr=%u", (uint32_t)emu68_usbmouse_enum_addr);
    kprintf_pc(__putc, NULL, "\n");
    kprintf_pc(__putc, NULL, "HID MOUSE     : %s\n", emu68_usbmouse_mounted ? "OK" : "NO");
    kprintf_pc(__putc, NULL, "REPORTS       : %u\n", emu68_usbmouse_report_count);
    kprintf_pc(__putc, NULL, "LAST DX/DY    : %d / %d\n",
               (int32_t)emu68_usbmouse_last_dx,
               (int32_t)emu68_usbmouse_last_dy);
    kprintf_pc(__putc, NULL, "\nBoot continues in 10 seconds...\n");

    /* Keep servicing USB while the final result remains photographable. */
    usbmouse_diag_poll_ms(10000);

    /* Movements made during diagnostics must not offset Workbench later. */
    emu68_usbmouse_clear_motion();
}
#endif
