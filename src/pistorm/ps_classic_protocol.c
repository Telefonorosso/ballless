...
void ps_housekeeper() 
{
    ...
    /*
     * USB mouse runtime service:
     *
     * Enumeration/bootstrap is serviced aggressively by start_rpi64.c before
     * the Amiga is released.  Once normal Emu68 runtime begins, CPU2 is the
     * sole DWC2/TinyUSB owner and services the USB transport at ~1 kHz.
     *
     * The USB mouse module independently publishes accumulated motion to the
     * virtual JOY0DAT counters at 50 Hz, so USB protocol servicing is not
     * throttled to the Amiga-visible pointer cadence.
     *
     * Nothing USB is called from the JIT, vectors.c, or the JOY0DAT read path.
     */
    const uint64_t usbmouse_poll_ticks = (tmp >= 1000) ? (tmp / 1000) : 1;
    uint64_t usbmouse_last_poll;
    asm volatile("mrs %0, CNTPCT_EL0":"=r"(usbmouse_last_poll));
    ...
    for(;;) {
        ...
        {
            uint64_t usbmouse_now;
            asm volatile("mrs %0, CNTPCT_EL0":"=r"(usbmouse_now));

            if ((usbmouse_now - usbmouse_last_poll) >= usbmouse_poll_ticks) {
                usbmouse_last_poll = usbmouse_now;
